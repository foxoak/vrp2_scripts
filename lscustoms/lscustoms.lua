										...
										if pos.locked then
											...
											if not LSC_Config.lock then
												if IsControlJustPressed(1,201) then
													...
													drawTxt("Aperte ~b~ENTER~w~ to enter ~b~Los Santos Customs ",4,1,0.5,0.8,1.0,255,255,255,255)
												end
											else
												drawTxt("~r~Em uso, aguarde",4,1,0.5,0.8,1.0,255,255,255,255)
											end
										else
											if IsControlJustPressed(1,201) then
												inside = true
												...
												drawTxt("Aperte ~b~ENTER~w~ para entrar na ~b~Los Santos Customs ",4,1,0.5,0.8,1.0,255,255,255,255)
											end
										end
									else
										drawTxt("~r~Este carro não pode ser tunado",4,1,0.5,0.8,1.0,255,255,255,255)
									end
								end
							else
...
--Setup main menu
LSCMenu = Menu.new("Los Santos Customs","CATEGORIES", 0.16,0.13,0.24,0.36,0,{255,255,255,255})
...
function DriveInGarage()
	...
	--print('Oi Current')
	--print(currentgarage)
	--vRP.EXT.lscustoms:lockGarage_server(true,currentgarage)
	...
	if DoesEntityExist(veh) then
		...
		if currentgarage == 4 or currentgarage == 5 then
			LSCMenu:setTitle("Beeker's Garagem")
		...
		AddMod(0,LSCMenu.categories,"SPOILER", "Spoiler", "Aumentar a downforce.",true)
		AddMod(3,LSCMenu.categories,"SKIRTS", "Skirts", "Melhore a aparência do seu veículo com saias laterais personalizadas.",true)
		AddMod(4,LSCMenu.categories,"EXHAUST", "Exhausts", "Exaustores esportivos personalizados.",true)
		AddMod(6,LSCMenu.categories,"GRILLE", "Grille", "Melhor refrigeração do motor.",true)
		AddMod(7,LSCMenu.categories,"HOOD", "Hood", "Melhore o resfriamento do motor do carro.",true)
		AddMod(8,LSCMenu.categories,"FENDERS", "Fenders", "Melhore o painel do corpo com defensas personalizadas.",true)
		AddMod(10,LSCMenu.categories,"ROOF", "Roof", "Abaixe seu centro de gravidade com painéis de teto leves.",true)
		AddMod(12,LSCMenu.categories,"BRAKES", "Brakes", "Aumentar o poder de travagem e eliminar o enfraquecimento do freio.",true)
		AddMod(13,LSCMenu.categories,"TRANSMISSION", "Transmission", "Melhor aceleração com transmissão de relação estreita.",true)
		AddMod(14,LSCMenu.categories,"HORN", "Horn", "Buzinas customizadas.",true)
		AddMod(15,LSCMenu.categories,"SUSPENSION","Suspension","Atualize para uma configuração de suspensão orientada para esportes.",true)
		AddMod(16,LSCMenu.categories,"ARMOR","Armor","Proteja os ocupantes do seu carro com painéis compostos de especificações militares.",true)
		AddMod(18, LSCMenu.categories, "TURBO", "Turbo", "Reduza o turbocompressor de atraso.",false)
		...
		if chassis then
			...
			AddMod(5,LSCMenu.categories.Chassis,"ROLL CAGE", "Roll cage", "Enrijeça o seu chassi com uma gaiola.",true)
		end
		
		LSCMenu.categories:addSubMenu("ENGINE", "Engine",nil, true)
		AddMod(39, LSCMenu.categories.Engine, "ENGINE BLOCK", "Engine Block", "Carcaças do bloco do motor personalizadas.",true)
		AddMod(40, LSCMenu.categories.Engine, "CAM COVER", "Cam Cover", "Tampa de válvula opcional",true)
		AddMod(41, LSCMenu.categories.Engine, "STRUT BRACE", "Strut Brace", "Uma seleção de suportes de apoio.",true)
		AddMod(11,LSCMenu.categories.Engine,"ENGINE TUNES", "Engine Tunes", "Aumenta a potência.",true)
		
		if interior then
			LSCMenu.categories:addSubMenu("INTERIOR", "Interior","Produtos para máximo estilo e conforto.", true)
			--LSCMenu.categories.Interior:addSubMenu("TRIM", "Trim","A selection of interior designs.", true)
			AddMod(27, LSCMenu.categories.Interior, "TRIM DESIGN", "Design de guarnição", "",true)
			...
			AddMod(28, LSCMenu.categories.Interior, "ORNAMENTS", "Ornaments", "Adicione itens decorativos ao seu painel.",true)
			AddMod(29, LSCMenu.categories.Interior, "DASHBOARD", "Dashboard", "Projetos de painéis de controle personalizados.",true)
			AddMod(30, LSCMenu.categories.Interior, "DIAL DESIGN", "Dials", "Personalize a aparência dos seus mostradores.",true)
			AddMod(31, LSCMenu.categories.Interior, "DOORS", "Doors", "Instale os upgrades das portas.",true)
			AddMod(32, LSCMenu.categories.Interior, "SEATS", "Seats", "Opções onde o estilo encontra o conforto.",true)
			AddMod(33, LSCMenu.categories.Interior, "STEERING WHEELS", "Steering Wheels", "Personalize o link entre você e seu veículo.",true)
			AddMod(34, LSCMenu.categories.Interior, "Shifter leavers", "Mudanças de câmbio", "",true)
		...
		LSCMenu.categories:addSubMenu("PLATES", "Plates","Identificação decorativa.", true)
		...
		AddMod(48,LSCMenu.categories,"Liveries", "Liveries", "Uma seleção de decalques para o seu veículo.",true)
		
		if bumper then
			LSCMenu.categories:addSubMenu("BUMPERS", "Bumpers", "Pára-choques dianteiro e traseiro personalizados",true)
			if fbumper then
				AddMod(1,LSCMenu.categories.Bumpers,"FRONT BUMPERS", "Front bumpers", "Pára-choques dianteiros personalizados.",true)
			end
			if rbumper then
				AddMod(2,LSCMenu.categories.Bumpers,"REAR BUMPERS", "Rear bumpers", "Pára-choques traseiros personalizados.",true)
			end
		end
		
		local m = LSCMenu.categories:addSubMenu("LIGHTS", "Lights", "Maior visibilidade noturna.",true)
		...
		respray = LSCMenu.categories:addSubMenu("RESPRAY", "Respray", "Transforma a aparência do veículo.",true)
		...
		LSCMenu.categories:addSubMenu("WHEELS", "Wheels", "rodas, pneus e cores personalizados.",true)
			wtype = LSCMenu.categories.Wheels:addSubMenu("WHEEL TYPE", "Wheel type", "Rodas personalizadas em todos os estilos e tamanhos.",true)
		...
		m = LSCMenu.categories.Wheels:addSubMenu("WHEEL COLOR", "Wheel color", "Cores de roda personalizada.",true)
		...
		m = LSCMenu.categories.Wheels:addSubMenu("WHEEL ACCESSORIES", "Wheel accessories", "Pneus à prova de balas e fumaça de burnout personalizada.",true)
		...
		m = LSCMenu.categories:addSubMenu("WINDOWS", "Windows", "Uma seleção de janelas coloridas.",true)	
...
function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
		...
		--vRP.EXT.lscustoms:LSC_finished(myveh)
		...
		--Unlock the garage
		--vRP.EXT.lscustoms:lockGarage_server(false,currentgarage)
...
function LSCMenu:onButtonSelected(name, button)
	...
	vRP.EXT.lscustoms.remote.LSC_buttonSelected_S(name, button)
end

--So we get the button back from server +  bool that determines if we can prchase specific item or not
function lscustoms:LSC_buttonSelected(name, button, canpurchase)
	name = name:lower()
	local m = LSCMenu.currentmenu
	local price = button.price or 0
	local veh = myveh.vehicle
...
function lscustoms:lscustom_sayPayment(tbl)
	waitingPayment = tbl
end
...
function CanPurchase(price, canpurchase)
	if canpurchase then
		vRP.EXT.lscustoms.remote._lscustom_doPayment(price)
		--TriggerServerEvent("lscustom:doPayment",price)
			...
			if waitingPayment == 2 then
				if LSCMenu.currentmenu == "main" then
					LSCMenu:showNotification("Seu veiculo foi consertado.")
				else
					LSCMenu:showNotification("Peça comprada.")
				end
				waitingPayment = 0
				return true
			elseif waitingPayment == 3 then
				LSCMenu:showNotification("~r~Você não pode pagar por esse item.")
				waitingPayment = 0
				return false
			end
	else
...
function lscustoms:lockGarage(tbl)
...
lscustoms.tunnel = {}
lscustoms.tunnel.LSC_buttonSelected = lscustoms.LSC_buttonSelected
lscustoms.tunnel.lscustom_sayPayment = lscustoms.lscustom_sayPayment
lscustoms.tunnel.lockGarage = lscustoms.lockGarage
